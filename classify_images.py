...
def classify_images(images_dir, results_dic, model):
    ...
    # Iterate through results_dic processing each pet image (filename) with the 
    # classifier function to get the classifier label.
    
    for filename, pet_label in results_dic.items():
            
        # Get Classifier Labels from Classifier Function:
        
        # Concatenate string to get full path
        full_path = images_dir + filename
       
        # Processing each pet image (filename) with the classifier function to get the         # classifier label
        model_label = classifier(full_path, model)

        # Format/process string to lowercase letters and remove whitespace characters
